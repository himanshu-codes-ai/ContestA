...
- **Global scope**: `C:/ContestA/.windsurf/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.windsurf/notes/`)
...
2. Glob `C:/ContestA/.windsurf/notes/*.md` (if directory exists) — global notes
...
Global (C:/ContestA/.windsurf/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.windsurf/notes/` — works in any directory
