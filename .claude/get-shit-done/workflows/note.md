...
- **Global scope**: `C:/ContestA/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.claude/notes/`)
...
2. Glob `C:/ContestA/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/ContestA/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.claude/notes/` — works in any directory
