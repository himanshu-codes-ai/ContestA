...
- **Global scope**: `C:/ContestA/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.gemini/notes/`)
...
2. Glob `C:/ContestA/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/ContestA/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.gemini/notes/` — works in any directory
