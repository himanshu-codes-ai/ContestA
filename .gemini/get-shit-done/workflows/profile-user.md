...
Key references:
- @C:/ContestA/.gemini/get-shit-done/references/ui-brand.md (display patterns)
- @C:/ContestA/.gemini/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @C:/ContestA/.gemini/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md"
...
```bash
cp "C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md" "C:/ContestA/.gemini/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `C:/ContestA/.gemini/get-shit-done/references/user-profiling.md`
...
Reference: @C:/ContestA/.gemini/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to C:/ContestA/.gemini/CLAUDE.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd:dev-preferences at C:/ContestA/.gemini/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node C:/ContestA/.gemini/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to C:/ContestA/.gemini/CLAUDE.md"
...
```bash
BACKUP_PATH="C:/ContestA/.gemini/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    C:/ContestA/.gemini/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd:dev-preferences   C:/ContestA/.gemini/commands/gsd/dev-preferences.md
  ✓ CLAUDE.md section       ./CLAUDE.md
  ✓ Global CLAUDE.md        C:/ContestA/.gemini/CLAUDE.md
```
