...
- **Global scope**: `C:/ContestA/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.cursor/notes/`)
...
2. Glob `C:/ContestA/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (C:/ContestA/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.cursor/notes/` — works in any directory
