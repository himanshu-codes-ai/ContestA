...
- **Global scope**: `C:/ContestA/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.codex/notes/`)
...
2. Glob `C:/ContestA/.codex/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.codex/notes/` — works in any directory
