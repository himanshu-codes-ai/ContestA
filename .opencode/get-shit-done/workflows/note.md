...
- **Global scope**: `C:/ContestA/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/ContestA/.opencode/notes/`)
...
2. Glob `C:/ContestA/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (C:/ContestA/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/ContestA/.opencode/notes/` — works in any directory
